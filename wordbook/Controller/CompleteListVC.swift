...
import RealmSwift
  ...
  class CompleteListVC : UIViewController,UICollectionViewDelegate, UICollectionViewDelegateFlowLayout, CompleteListCellDelegate {
  ...
  override func viewDidLoad() {
    ...
    collectionView.register(UINib(nibName: "CompleteListCell", bundle: nil), forCellWithReuseIdentifier: "completeCell")
  ...
  func extractFlashcards(withLevel level : Int, withSegment segment : Int){
      ...
      case 0 : flashcards = realm.objects(FlashCards.self).filter("completion = 1 && category = 0")
      case 1 : flashcards = realm.objects(FlashCards.self).filter("completion = 1 && category = 1")
      case 2 : flashcards = realm.objects(FlashCards.self).filter("completion = 1 && category = 2")
      case 3 : flashcards = realm.objects(FlashCards.self).filter("completion = 1 && category = 3")
      default :  flashcards = realm.objects(FlashCards.self).filter("completion = 1 && category = 0")
        
    }
  }
}

extension CompleteListVC : UICollectionViewDataSource {
  ...
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "completeCell", for: indexPath) as? CompleteListCell {
